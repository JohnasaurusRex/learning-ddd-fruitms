// src/modules/fruits/application/useCases/storeFruit/storeFruit.ts
...
interface StoreFruitDTO {
  name: string;
  amount: number;
}

export class StoreFruitUseCase implements UseCase<StoreFruitDTO, Promise<Result<void>>> {
  ...
  async execute(request: StoreFruitDTO): Promise<Result<void>> {
    try {
      ...
      const storeResult = fruit.storeFruit(amountOrError.getValue());
      if (storeResult.isFailure) {
        return Result.fail<void>(storeResult.error as string);
      }
